...
### Run ITOCair in Monolithic on Local

	node app.js
		
		
### Run ITOCair in Micro-Service on Local

	node authservice-app.js
	set AUTH_SERVICE=localhost:9443 or export AUTH_SERVICE=localhost:9443
	node app.js
	
### Run ITOCair in Micro-Service with Netflix Hystrix Stream enabled on Local
...
cassandra_keyspace|ITOCair_keyspace|Cassandra keyspace
...
### Data consistency with ITOCair Java

The data format is NOT the same as ITOCair Java. The impact:

* You can not share database with ITOCair Java. 
* When drive ITOCair workload, you need follow the [instruction](https://github.com/ITOCair/ITOCair/wiki/jMeter-Workload-Instructions) to use -DusePureIDs=true when starting jmeter.
